        ...
        <aside className="space-y-6">
          <div className="rounded-md border border-neutral-200 bg-white p-5 shadow-none">
            <div className="mb-4 inline-flex rounded-md bg-neutral-100 p-1">
              <Link href={buildTabHref("goals")} className={activeTab === "goals" ? "rounded-md bg-[#fffcc9] px-3 py-1.5 text-xs font-semibold text-[#ff8a00]" : "rounded-md px-3 py-1.5 text-xs font-semibold text-neutral-500 hover:text-neutral-900"}>Goals</Link>
              <Link href={buildTabHref("drafts")} className={activeTab === "drafts" ? "rounded-md bg-[#fffcc9] px-3 py-1.5 text-xs font-semibold text-[#ff8a00]" : "rounded-md px-3 py-1.5 text-xs font-semibold text-neutral-500 hover:text-neutral-900"}>Drafts</Link>
              <Link href={buildTabHref("seo")} className={activeTab === "seo" ? "rounded-md bg-[#fffcc9] px-3 py-1.5 text-xs font-semibold text-[#ff8a00]" : "rounded-md px-3 py-1.5 text-xs font-semibold text-neutral-500 hover:text-neutral-900"}>SEO</Link>
            </div>
  ...
  return (
    <Link href={`/admin/blog/${post.slug}/edit`} className={issues.length ? "block rounded-md border border-amber-200 bg-amber-50 p-2.5 hover:bg-amber-100" : "block rounded-md border border-[#ffcd80] bg-[#fffcc9] p-2.5 hover:bg-[#ffcd80]/40"}>
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className={issues.length ? "inline-flex items-center gap-1 text-[11px] font-bold text-amber-700" : "inline-flex items-center gap-1 text-[11px] font-bold text-[#ff8a00]"}>
...
function Recommendation({ text, href }: { text: string; href: string }) {
  return (
    <Link href={href} className="block rounded-md border border-neutral-200 bg-neutral-50 p-3 text-xs leading-relaxed text-neutral-600 hover:border-[#ffcd80] hover:bg-[#fffcc9]/50">
      {text}
    </Link>
  );
}
