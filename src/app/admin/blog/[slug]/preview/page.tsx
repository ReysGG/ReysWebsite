...
export default async function AdminBlogPreviewPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f3ff] pt-6 pb-24 text-neutral-950">
      <div className="mx-auto mb-4 flex max-w-4xl items-center justify-between px-6 md:px-0">
        <Link
          href={`/admin/blog/${post.slug}/edit`}
          className="inline-flex items-center gap-2 rounded-md border border-neutral-200 bg-white px-3 py-2 text-sm font-semibold text-neutral-700 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700"
        >
          <ArrowLeft className="h-4 w-4" />
          Kembali edit
        </Link>
        <div className="inline-flex items-center gap-2 rounded-full border border-indigo-100 bg-white px-3 py-1.5 text-xs font-semibold uppercase tracking-widest text-indigo-700">
