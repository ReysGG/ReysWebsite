...
export function getServicesEditFields(config: SiteConfig): InlineEditField[] {
  ...
  return [
    ...
    ...config.services.items.flatMap((item, index) => [
      { section, label: `Layanan ${index + 1} nomor`, name: `services.${index}.number`, value: item.number },
      { section, label: `Layanan ${index + 1} judul`, name: `services.${index}.title`, value: item.title },
      { section, label: `Layanan ${index + 1} deskripsi`, name: `services.${index}.description`, value: item.description, textarea: true },
    ]),
  ];
}
