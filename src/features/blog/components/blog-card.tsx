...
export function BlogCard({ post }: { post: BlogPost }) {
  return <Link href={`/blog/${post.slug}`} className="group flex min-h-[420px] flex-col overflow-hidden rounded-3xl border border-indigo-100 bg-white/90 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
    ...
    <div className="flex flex-1 flex-col p-6">
      <div className="mb-4 flex flex-wrap items-center gap-3 text-xs font-semibold text-neutral-500">{post.category && <span className="rounded-full bg-indigo-50 px-3 py-1 text-indigo-700">{post.category}</span>}<span className="inline-flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" />{formatBlogDate(getPostDate(post))}</span><span>{getReadingTime(post)}</span>{post.featured && <span className="text-indigo-600">Featured</span>}</div>
