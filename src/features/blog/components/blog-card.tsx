...
export function BlogCard({ post }: { post: BlogPost }) {
  return <Link href={`/blog/${post.slug}`} className="group flex min-h-[420px] flex-col overflow-hidden rounded-3xl border border-indigo-100 bg-white/90 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
    {post.coverImage ? <div className="relative h-52 overflow-hidden bg-indigo-50"><Image src={post.coverImage} alt={post.title} fill className="object-cover transition duration-700 group-hover:scale-105" /></div> : <div className="flex h-52 items-center justify-center bg-gradient-to-br from-indigo-600 to-violet-600 text-white"><FileText className="h-10 w-10" /></div>}
