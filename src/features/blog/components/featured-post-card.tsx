...
export function FeaturedPostCard({ post }: { post: BlogPost }) {
  return <section className="mb-8 overflow-hidden rounded-3xl border border-indigo-100 bg-white shadow-sm lg:grid lg:grid-cols-2">
    <div className="relative min-h-72 bg-gradient-to-br from-indigo-600 to-violet-600">{post.coverImage && <Image src={post.coverImage} alt={post.title} fill unoptimized className="object-cover" />}</div>
